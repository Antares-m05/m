...
class BasicBlockWithCBAM(BasicBlock):
    def __init__(self, *args, reduction=4, **kwargs):
        super().__init__(*args, **kwargs)
        self.cbam = CBAM(self.conv2.out_channels, reduction_ratio=reduction)
...
class ResNet34_CBAM(ResNet):
    def __init__(self):
        super().__init__(
            block=BasicBlockWithCBAM,
            layers=[3, 4, 6, 3],  #resnet34
            # layers = [2, 2, 2, 2]  # resnet18  最好acc（5*50，repeat=3  acc=69%）
        )
