        ...
        for cls in sorted(os.listdir(root)):
            ...
            for f in sorted(os.listdir(cls_dir)):
                if f.endswith(".mat"):
                    ...
                    # snr = np.log10(snr)
                    #snr = (snr - snr.min()) / (snr.max() - snr.min()) #加上74%，去掉78
...
class ResNetBackbone(nn.Module):
    def __init__(self):
        ...
        #
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=(2,2), stride=1, padding=1)  # 改了输入的
        #
        self.bn1 = resnet.bn1
...
class ResNetWithAttention(nn.Module):
    def __init__(self, emb_dim=128):
        ...
        self.base1.conv1 = nn.Conv2d(1, 64, kernel_size=(1, 1), stride=1, padding=2, bias=False)

        self.base2 = ResNetBackbone()
        self.base2.conv1 = nn.Conv2d(1, 64, kernel_size=(4, 4), stride=1, padding=2, bias=False)

        self.base3 = ResNetBackbone()
        self.base3.conv1 = nn.Conv2d(1, 64, kernel_size=(8, 8), stride=1, padding=2, bias=False)
        ...
        # ========= 融合后注意力 =========
        self.cbam = CBAM(256 * 3, reduction_ratio=1)

        # ========= 语义关系模块（Lite Non-local） =========
        self.semantic_pool = nn.AdaptiveAvgPool2d((8, 7))
        self.semantic = SemanticRelationLite(256 * 3, reduction=4)
    ...
    def forward(self, x):
        ...
        xs = self.semantic(xc)             # semantic reasoning
...
def main():
    ...
    train_set = RadarDataset(r"E:\yan\新科楼顶数据集\切片16x16\train")
    test_set  = RadarDataset(r"E:\yan\新科楼顶数据集\切片16x16\test")

    train_loader = make_fsl_loader(train_set, 4, 5, 5, 500)
    test_loader  = make_fsl_loader(test_set, 4, 5, 5, 200)
    ...
    print("=== Visualizing RD & Branch Outputs ===")
    visualize_rd_and_branches(backbone, sample)
    acc = t_fsl(model, test_loader, device)
    return acc
...
def visualize_rd_and_branches(model, sample):
    ...
    def feature_map(feat):
        ...
        fmap = (fmap - fmap.min()) / (fmap.max() - fmap.min() + 1e-10)
        return fmap
